...
* The following CPAN Perl modules are dependencies that will be installed
  ...
  * `Bioperl` modules (>=1.6.901). Note that some unreleased Bioperl modules
    have been included in Biogrinder.
  * `Getopt::Euclid` (>= 0.3.4)
...
If you don't have administrator privileges, then Biogrinder needs to be installed
...
install Biogrinder by following the instructions detailed as above.

## Running Biogrinder

You can run Biogrinder using the

* command-line interface (CLI). see ```grinder --help```
* programming interface (API). see ```perldoc Biogrinder```
...
It's included in the Biogrinder package and contains the following utilities:

* `average genome size`: This calculates the average genome size (in bp) of a
    simulated random library produced by Biogrinder.
...
A variety of FASTA databases can be used as input for Biogrinder. For example,
...
The Biogrinder API allows to conveniently use Biogrinder within Perl scripts. Here is
a synopsis:
...
my $factory = Biogrinder->new( -reference_file => 'genomes.fna' );
...
while ( my $struct = $factory->next_lib ) {
  ...
  while ( my $read = $factory->next_read) {
    ...
    my $read_id     = $read->id;     # read ID given by Biogrinder
...
Bug reports, suggestions and patches are welcome. Biogrinder's code is developed on
